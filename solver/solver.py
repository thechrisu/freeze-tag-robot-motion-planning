...
class Solution(object):
    ...
    def reach_closest_robot(self, start, current_path):
        global total_robots_reached
        print(total_robots_reached)
        total_robots_reached += 1
        ...
        t1 = threading.Thread(target=self.reach_closest_robot, args=(robot, current_path + [robot]))
        t2 = threading.Thread(target=self.reach_closest_robot, args=(robot, [robot]))
...
with open('solver.mat', 'a') as sol_file:
    ...
    for p_ind in probs_to_solve:
        ...
        print(sol_str)
        with open('intermediate-' + str(sol.problem_number) + 'mat', 'a') as safety_file:
            safety_file.writelines(sol_str + '\n')
